    ...
    def fit(self, model, input_data, device):
        ...
        with torch.no_grad():
            ...
            for (reward_func, reward_weight) in zip(self.reward_funcs, self.reward_weights): 
                ...
                if reward_func == 'ndcg':
                    R = torch.gather(2 ** rank_label - 1, dim = 1, index = ranking)
...
class PLRank3(BaseRLAlgo):
    ...
    def fit(self, model, input_data, device):
        ...
        ranked_scores = torch.gather(rank_score, dim = 1, index = ranking)
        ...
        with torch.no_grad():
            ...
            exp_scores = torch.exp(ranked_scores)
            S = torch.flip(torch.cumsum(torch.flip(exp_scores, dims= [1]), dim = 1), dims = [1])
            RI = PR / S
            DR = discount / S
            RI = torch.cumsum(RI, dim = 1)
            DR = torch.cumsum(DR, dim = 1)
            
            reward = PR - temp + exp_scores * rho * DR - exp_scores * RI

        loss = -torch.sum(ranked_scores * reward, dim=1).mean()
